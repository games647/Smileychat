 ...
 * <p/>
 * An example of a good configuration option would be something similar to 'auto-update: true' - if this value is set to false you may NOT run the auto-UpdateChecker.
 * <br>
...
public class UpdateChecker {
    ...
    public UpdateChecker(Plugin plugin, int id, File file, UpdateType type, boolean announce) {
        ...
        try {
            this.url = new URL(UpdateChecker.HOST + UpdateChecker.QUERY + id);
    ...
    private void saveFile(File folder, String file, String u) {
        ...
        try {
            ...
            final byte[] data = new byte[UpdateChecker.BYTE_SIZE];
            int count;
            if (this.announce) {
                this.plugin.getLogger().info("About to download a new update: " + this.versionName);
            }
            long downloaded = 0;
            while ((count = in.read(data, 0, UpdateChecker.BYTE_SIZE)) != -1) {
                downloaded += count;
            ...
            this.result = UpdateChecker.UpdateResult.FAIL_DOWNLOAD;
        } finally {
            try {
                if (in != null) {
                    in.close();
        ...
        try {
            ...
            while (e.hasMoreElements()) {
                ...
                if (entry.isDirectory()) {
                    ...
                    final BufferedOutputStream bos = new BufferedOutputStream(fos, UpdateChecker.BYTE_SIZE);
                    while ((b = bis.read(buffer, 0, UpdateChecker.BYTE_SIZE)) != -1) {
    ...
    private boolean versionCheck(String title) {
        if (this.type != UpdateType.NO_VERSION_CHECK) {
            ...
            if (title.split(" v").length == 2) {
                ...
                this.result = UpdateChecker.UpdateResult.FAIL_NOVERSION;
                return false;
            }
        }
        return true;
    ...
    private boolean hasTag(String version) {
        for (final String string : UpdateChecker.NO_UPDATE_TAG) {
    ...
    private boolean read() {
        try {
            ...
            this.versionName = (String) ((JSONObject) array.get(array.size() - 1)).get(UpdateChecker.TITLE_VALUE);
            this.versionLink = (String) ((JSONObject) array.get(array.size() - 1)).get(UpdateChecker.LINK_VALUE);
            this.versionType = (String) ((JSONObject) array.get(array.size() - 1)).get(UpdateChecker.TYPE_VALUE);
            this.versionGameVersion = (String) ((JSONObject) array.get(array.size() - 1)).get(UpdateChecker.VERSION_VALUE);
    ...
    private class UpdateRunnable implements Runnable {
        ...
        @Override
        public void run() {
            if (UpdateChecker.this.url != null) {
                // Obtain the results of the project's file feed
                if (UpdateChecker.this.read()) {
                    if (UpdateChecker.this.versionCheck(UpdateChecker.this.versionName)) {
                        if ((UpdateChecker.this.versionLink != null) && (UpdateChecker.this.type != UpdateType.NO_DOWNLOAD)) {
                            ...
                            UpdateChecker.this.saveFile(new File(UpdateChecker.this.plugin.getDataFolder().getParent(), UpdateChecker.this.updateFolder), name, UpdateChecker.this.versionLink);
                        } else {
                            UpdateChecker.this.result = UpdateResult.UPDATE_AVAILABLE;
                        }
                    }
                }
            }
